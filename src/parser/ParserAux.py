"This script defines the process of parsing the raw radiometric data (AMR, ACT, SND channels)"
             ...
             if CompleteLine==True:
                ...
                #The line is not complete
                 if (line[-6:-1]!=self.EndHeader):
                      ...
                      print(package,'->', time.strftime("%b-%d-%Y -- %H:%M:%S",time.localtime(int(timestamp))), '->', PackageNumber, 'DATA LENGTH:', datapackagelength)
                  ...
                  # Line complete
                        ...
                        iterData+=line[:-6]
                        datapackagelength=len(iterData)
                        print (self.package,'->', time.strftime("%b-%d-%Y -- %H:%M:%S",time.localtime(int(timestamp))), '->', PackageNumber, 'DATA LENGTH:', datapackagelength)
                        self.toplot=np.append(self.toplot,PackageNumber)
                        self.datalength=np.append(self.datalength, datapackagelength)
                        to5hparser=''.join([leftvalues, iterData])
                        leftvalues=self.actualh5parser(timestamp,to5hparser,PackageNumber)
...
class RadiometerParser(definedParser):
    ...
    def actualh5parser(self, timestamp, inputvalues, PackageNumber):
        ...
        while leftvalues > self.bytesPerDatagram_SND:
            ...
            if header == self.MW_HEADERARRAY:
                print('reading MW package', self.x_AMR, 'header: ', struct.unpack('>1B', self.c), struct.unpack('>1B', b), struct.unpack('>1B', self.a))
                indexstart = index
    ...
    class ThermistorParser(definedParser):
        ...
        def actualh5parser(self, timestamp, inputvalues, PackageNumber):
             ...
             if self.verbose == True:
                  print('->', time.strftime("%b-%d-%Y -- %H:%M:%S", time.localtime(int(timestamp))),
...
class GPSParser(definedParser):
    ...
    def CreateFile(filename):
        ...
        if OperatingSystem == "Windows":
            h5file = tb.open_file(filename, mode="w", title="Acquisition data")
            groupR = h5file.create_group("/", 'Radiometric_Data', 'Data from microwave (6 channels), millimeter-wave (3 channels) and sounders (16 channels) + motor position + computer time')
            groupT = h5file.create_group("/", 'Temperature_Data', 'Thermistor readout + computer time')
            groupG = h5file.create_group("/", 'GPS_IMUData', 'Euler angles {Roll, Pitch and Yaw} + GPS position {Lat, Lon, Height} + GPS time + computer time')
            groupI = h5file.create_group("/", 'Information', 'Please read this section before proceeding to read the file')
            
            tableAMR = h5file.create_table(groupR, 'MW_DATA', h5classes.AMRsample, "Radiometric data from AMR")
            tableACT = h5file.create_table(groupR, 'MMW_DATA', h5classes.ACTsample, "Radiometric data from ACT")
            tableSND = h5file.create_table(groupR, 'SND_DATA', h5classes.SNDsample, "Radiometric data from SND")
            tableTHR = h5file.create_table(groupT, 'Thermistor_DATA', h5classes.Thermistorsample, "System's Temperature in volts (5 kOhm thermistors)")
            tableIMU = h5file.create_table(groupG, 'GPSIMU_DATA', h5classes.IMUsample, "System's attitude, position in lat.-lon. and GPS time")

            tableINFGeneral = h5file.create_table(groupI, 'General_INFO', h5classes.Information, "Raw files used for composing this h5 file")
            tableINFServer = h5file.create_table(groupI, 'Server_INFO', h5classes.Information, "Information regarding the server in .conf format (JSON)")
            tableINFRadiometer = h5file.create_table(groupI, 'Radiometer_INFO', h5classes.Information, "Generic information of the radiometer")
            tableINFThermistors = h5file.create_table(groupI, 'Thermistors_INFO', h5classes.Information, "Generic information of the thermistors")
            tableINFGPS = h5file.create_table(groupI, 'GPS_INFO', h5classes.Information, "Generic information of the GPS-IMU")
       ...
       # Initialize rows
        sampleAMR = tableAMR.row
        sampleACT = tableACT.row
        sampleSND = tableSND.row
        sampleTHR = tableTHR.row
        sampleIMU = tableIMU.row

        sINFServer = tableINFServer.row
        sINFGeneral = tableINFGeneral.row
        sINFRadiometer = tableINFRadiometer.row
        sINFThermistors = tableINFThermistors.row
        sINFGPS = tableINFGPS.row
        
        # Thermistor calibration information
        sINFThermistors['General'] = ('Coefficients from voltage to Temperatures for KS502J2 thermistors: ''A = 1.29337828808e-3, B = 2.34313147501e-4, C = 1.09840791237e-7, D = -6.51108048031e-11')
        sINFThermistors.append()

        sINFThermistors['General'] = ('Coefficients from voltage to Temperatures for 44906 thermistors (used in AMR): ' 'A = 1.28082086269172e-3, B = 2.36865057309759e-4, C = 9.02634799967035e-8, D = 0')
        sINFThermistors.append()

        sINFThermistors['General'] = ('regulatedVoltage = 1.06, resist = (MeasuredVoltage / (regulatedVoltage - MeasuredVoltage)) * 5000')
        sINFThermistors.append()

        sINFThermistors['General'] = ('tempInv = (A + B * log(resist) + C * log(resist)**3 + D * log(resist)**5), temp = 1 / tempInv')
        sINFThermistors.append()

        sINFThermistors['General'] = '''
        ...
        sINFThermistors.append()
        tableINFThermistors.flush()

        return (sampleAMR, sampleACT, sampleSND, sampleTHR, sampleIMU,sINFGeneral, sINFServer,
        tableAMR, tableACT, tableSND, tableTHR, tableIMU,
        tableINFGeneral, tableINFServer,
        h5file)       
