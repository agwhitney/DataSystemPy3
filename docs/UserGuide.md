...
## Configure AMR
1. Run the AMR server application in the background. This configures the FPGA and begins streaming data that will be collected by the client application. There should be no need to stop or restart it.
    * `uv run hammr/masterserver.py &`
    * The `&` at the end sends the command to the background.

## Configure HMS
1. In the HMS window, send the confguration to the FPGA. This will take some time (~ 60s).
    * `./client-remote-config -i 192.168.137.110 -a RUN_SEQ -f ./init_18G_hyms_eng_mode.seq`
    * There was an error that running this more than once would lock the FPGA into a non-measuring state. This has been fixed, but try to avoid it.
